...
function buildStatementPrompt({ statement, mode, facts, rules }) {
    ...
    return [
        ...
        '',
        'Statement:',
        `"${statement}"`, // Corrected: escaped quote within template literal
        '',
        'Relevant rules:',
...
function buildDocumentPrompt({ document, mode, facts, rules, maxHighlights }) {
    ...
    return [
        ...
        '',
        'Document:',
        '"""', // Corrected: escaped triple quotes
        document,
        '"""', // Corrected: escaped triple quotes
        '',
        'Relevant rules:',
        ...
        '',
        `${instruction}`,
        `Identify up to ${maxHighlights} key findings.`, // Corrected: escaped quote within template literal
        'Respond strictly in JSON with the following structure:',
        '{',
        '  "verdict": "supported|contradicted|mixed|insufficient",',
...
class RAGService {
    constructor({ client, llmAgent }) {
        if (!client || typeof client.execute !== 'function') {
            throw new Error('RAGService requires a valid Persisto client.');
        }
        if (!llmAgent) {
            throw new Error('RAGService requires a valid LLM agent.');
        }
        this.client = client;
        this.llmAgent = llmAgent;
    }

    async analyzeStatement({ statement, mode = 'audit', log = true }) {
        const trimmedStatement = toSafeString(statement);
        if (!trimmedStatement) {
            throw new Error('Statement is required.');
        }

        const allFacts = await fetchFacts(this.client, { limit: 200 });
        const allRules = await fetchRules(this.client, { limit: 120 });
        ...
        if (!ragAvailable) {
            return this.analyzeStatementWithoutRag({ statement: trimmedStatement, mode, log });
        }
        ...
        let parsed = null;
        try {
            const raw = await this.llmAgent.complete({ prompt, temperature: 0.1 });
        ...
        if (log) {
            await logAssessment(this.client, {
    ...
    async analyzeDocument({ document, mode = 'audit', maxHighlights = 8, log = true }) {
        ...
        if (!ragAvailable) {
            return this.analyzeDocumentWithoutRag({ document: trimmedDocument, mode, maxHighlights, log });
        }
    ...
    async analyzeStatementWithoutRag({ statement, mode, log = true }) {
        const prompt = [
            ...
            '',
            `Mode: ${mode}`,
            `Statement: "${statement}"`, // Corrected: escaped quote within template literal
            '',
        ...
        let rawResponse = '';
        try {
            rawResponse = await this.llmAgent.complete({ prompt, temperature: 0.2 });
    ...
    async analyzeDocumentWithoutRag({ document, mode, maxHighlights, log = true }) {
        const prompt = [
            'You are VeritasAI. The structured knowledge base is currently empty or unavailable.',
